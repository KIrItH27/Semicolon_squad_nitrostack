...
exports.MaintenanceTools = exports.scheduleMaintenanceSchema = exports.predictFailureSchema = exports.checkMachineHealthSchema = void 0;
exports.sendWorkOrderEmail = sendWorkOrderEmail;
exports.checkMachineHealthLogic = checkMachineHealthLogic;
exports.predictFailureLogic = predictFailureLogic;
exports.scheduleMaintenanceLogic = scheduleMaintenanceLogic;
const dotenv_1 = __importDefault(require("dotenv"));
dotenv_1.default.config();
// @ts-ignore
const nodemailer_1 = __importDefault(require("nodemailer"));
const zod_1 = require("zod");
exports.checkMachineHealthSchema = zod_1.z.object({
    machineId: zod_1.z.string().optional().describe('Machine ID (e.g. "CNC-01", "PRESS-04"). Omit for all.'),
});
exports.predictFailureSchema = zod_1.z.object({
    machineId: zod_1.z.string().describe('Target machine ID.'),
    timeframeHours: zod_1.z.number().optional().default(72),
});
exports.scheduleMaintenanceSchema = zod_1.z.object({
    machineId: zod_1.z.string().describe('Exact Machine ID to schedule (e.g. MCH-212).'),
    maintenanceType: zod_1.z.enum(['PREVENTATIVE', 'CORRECTIVE', 'EMERGENCY', 'INSPECTION']).optional().default('EMERGENCY'),
    scheduledDate: zod_1.z.string().optional().describe('Scheduled date/time for maintenance.'),
    priority: zod_1.z.enum(['LOW', 'MEDIUM', 'HIGH', 'CRITICAL']).optional().default('CRITICAL'),
    assignedTechnician: zod_1.z.string().optional().default('Alex Vance'),
    recipientEmail: zod_1.z.string().optional().default('cb.ai.u4aid25043@cb.students.amrita.edu'),
    technicianNotes: zod_1.z.string().optional().describe('Reason for maintenance or diagnostic notes.'),
});
...
function extractExactMachineId(input) {
    ...
    if (input?.machineId)
        return String(input.machineId).toUpperCase();
    if (input?.machine)
        return String(input.machine).toUpperCase();
    return 'MCH-UNKNOWN';
}
function parseMachineId(input) {
    if (!input)
        return 'MCH-256';
    ...
    if (input?.machine)
        return String(input.machine).toUpperCase();
    if (input?.machine_id)
        return String(input.machine_id).toUpperCase();
    if (input?.targetMachine)
        return String(input.targetMachine).toUpperCase();
    if (input?.id)
        return String(input.id).toUpperCase();
    const jsonStr = JSON.stringify(input);
    const match = jsonStr.match(/(MCH-[0-9]+|CNC-[0-9]+|PRESS-[0-9]+|ROBOT-[A-Z0-9-]+)/i);
    return match ? match[0].toUpperCase() : 'MCH-256';
}
async function sendWorkOrderEmail(targetEmail, workOrder) {
    try {
...
async function checkMachineHealthLogic(input) {
...
async function predictFailureLogic(input) {
    const machineId = input?.machineId || 'MCH-212';
    const isCritical = machineId.includes('212') || machineId.includes('ROBOT');
    return {
        success: true,
...
async function scheduleMaintenanceLogic(input) {
...
class MaintenanceTools {
    async checkMachineHealth(input) { return checkMachineHealthLogic(input); }
    async predictFailure(input) { return predictFailureLogic(input); }
    async scheduleMaintenance(input) { return scheduleMaintenanceLogic(input); }
}
